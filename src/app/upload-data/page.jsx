...
export default function UploadForm() {
  const [formData, setFormData] = useState({
    ...
    tedTalks: [{ title: '', description: '', credit: '', speaker: '', file: null }],
  ...
  const handleSubmit = async (e) => {
    ...
    types.forEach((type) => {
      formData[type].forEach((item, index) => {
        ...
        if(type === 'videos') body.append(`${type}[${index}][videoUrl]`, item.videoUrl || '')
        if (type === 'articles') body.append(`${type}[${index}][articleUrl]`, item.articleUrl || '')
        if (type === 'books') body.append(`${type}[${index}][bookUrl]`, item.bookUrl || '')
        if (type === 'tedTalks') body.append(`${type}[${index}][tedUrl]`, item.tedUrl || '')
        if (type === 'tedTalks') body.append(`${type}[${index}][speaker]`, item.speaker || '')
        if (type === 'books') body.append(`${type}[${index}][author]`, item.author || '')
        ...
        <div key={index} className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          ...
          />
          {
            type ==='videos' && (
              <input
                ...
                value={item.videoUrl}
                onChange={(e) => handleChange(type, index, 'videoUrl', e.target.value)}
              />
            )
          }
          {
            type ==='tedTalks' && (
              <input
                ...
                placeholder="TedUrl"
                value={item.tedUrl}
                onChange={(e) => handleChange(type, index, 'tedUrl', e.target.value)}
              />
            )
          }
          ...
          {type === 'tedTalks' && (
            <input
              ...
              placeholder="Speaker"
              value={item.speaker}
              onChange={(e) => handleChange(type, index, 'speaker', e.target.value)}
            />
          )}
          ...
          {type === 'books' && (
            <input
              ...
              placeholder="Author"
              value={item.author}
              onChange={(e) => handleChange(type, index, 'author', e.target.value)}
            />
          )}
          {type === 'articles' && (
            <input
              ...
              onChange={(e) => handleChange(type, index, 'articleUrl', e.target.value)}
            />
          )}
          {type === 'books' && (
            <input
              ...
              placeholder="BookUrl"
              value={item.bookUrl}
              onChange={(e) => handleChange(type, index, 'bookUrl', e.target.value)}
            />
          )}
          <input
            className="input border-2 p-2 rounded-2xl"
            placeholder="Credit"
            value={item.credit}
            onChange={(e) => handleChange(type, index, 'credit', e.target.value)}
          />
          <div className='flex flex-col gap-2'>
          <label htmlFor="" className='text-red-400 font-semibold ml-3'>Upload Image Or Vidoes</label>
          <input
            type="file"
    ...
    >
      ...
      <div className="flex flex-col gap-4">
        ...
        <select
          ...
          onChange={(e) => setFormData({ ...formData, yourPassion: e.target.value })}
